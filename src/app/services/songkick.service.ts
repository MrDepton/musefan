...
export class SongkickService {
  ...
  searchArtist(query: string) {
    ...
    return this.http
      .get(`https://api.songkick.com/api/3.0/search/artists.xml?apikey=${this.apiKey}&query=${query}`, {
        headers: headers,
        responseType: 'text'
      })
      .pipe(map(retData => {
  ...
  getUpcomingForArtist(artistId: string) {
    const headers = new HttpHeaders({'Content-Type': 'text'}).set('Accept', 'text');
    return this.http
      .get(`https://api.songkick.com/api/3.0/artists/${artistId}/calendar.xml?apikey=${this.apiKey}`, {
        headers: headers,
        responseType: 'text'
      })
      .pipe(map(retData => {
        const ret: any = XMLParser.xml2js(retData, {compact: true});
        if (ret.resultsPage._attributes.totalEntries > 0) {
          if (Array.isArray(ret.resultsPage.results.event)) {
